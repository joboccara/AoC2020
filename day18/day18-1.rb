...
def parse(tokens)
  ...
  tokens.each do |token|
    ...
    when 'operator'
      if outputs_stack.size >= 2 && !operators_stack.empty? && operators_stack.top != '('
...
=begin
def parse(tokens)
  current_ast = Node.new('root')
  tokens.each do |token|
    case token.type
    when 'digit'
      digit_node = Node.new(token.value)
      last_child = current_ast.children.last
      if last_child
        last_child.add_child(digit_node)
      else
        current_ast.add_child(digit_node)
      end
    when 'operator'
      operator_ast = Node.new(token.value)
      operator_ast.add_child(current_ast.children.pop)
      current_ast.add_child(operator_ast)
    when 'open bracket'
      bracket_ast = Node.new('bracket')
      last_child = current_ast.children.last
      if last_child
        last_child.add_child(bracket_ast)
      else
        current_ast.add_child(bracket_ast)
      end
      current_ast = bracket_ast
    when 'close bracket'
      current_ast = current_ast.father
    end
  end
  current_ast
end
